function index-of
	...
	if test $array_length -gt 0
		for index in (seq 1 $array_length)
			...
			switch "$argv[$index]"
				...
				case -o --occurrence
					set occurrence $argv[$next_index]
				case -h --help
					echo 'Use the index-of command as follows'
					echo 'index-of --string "abcdecabcqwe" --character "abc"'
					exit
			end
		end
	end
	if test "$input_string" != ''
		if test "$input_character" != ''
			...
			for string_index in (seq $input_string_length)
				...
				if test $input_string_length -ge (math $string_index + $input_character_length - 1)
					...
					if test "$current_comparison" = "$input_character"
						...
						set match_occurrence (math $match_occurrence + 1)
						if test $match_occurrence -eq $occurrence
							echo $found_index
							return $found_index
						end
			...
			echo 'Use the index-of command as follows'
			echo 'index-of --string "abcdecaqwe" --character "a"'
			exit			
		end
	else

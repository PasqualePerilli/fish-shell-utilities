function find-and-replace-all
	...
	if test $array_length -gt 0
		...
		set -l lines_to_modify (find-lines-in-file --file $file_to_read --line "$regex_to_search")
		...
		if test $lines_to_modify != ''
			...
			for index in (seq $howmany_lines)
				...
				update-file --file $file_to_read --line $lines_to_modify[$index] --value $replacement_value
			end
		else

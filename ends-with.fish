function ends-with
	...
	if test $array_length -gt 0
		...
		set -l input_string_length (count (string split '' $starting_string))
		set -l input_pattern_length (count (string split '' $search_pattern))
		set -l start_index (math $input_string_length - $input_pattern_length + 1)
		set -l sub_ending_string (string sub --start=$start_index --length=$input_pattern_length $starting_string)
		#echo $sub_ending_string
		if test $sub_ending_string = $search_pattern
			if test $quiet -eq 1
				echo 0
			end
			return 0
